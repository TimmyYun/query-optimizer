...
def gen_values(
    ...
    if dist == "uniform":
        ...
        if use_micro_dist:
            ...
            global_norm = rng.normal(
                loc=num_chunks / 2.0, scale=num_chunks / 6.0, size=n
            )
            macro_bins = np.clip(np.round(global_norm), 0, num_chunks - 1).astype(
                np.int64
            )

            # Micro: A perfect mini bell-curve centered exactly in the middle of the bucket (500)
            micro_noise = rng.normal(
                loc=bucket_size / 2.0, scale=bucket_size / 6.0, size=n
            )
...
def scan_min_max_count(
    csv_path: Path, chunksize: int = 1_000_000
) -> Tuple[int, int, int]:
    """
...
def freedman_diaconis_bins(
    sample: np.ndarray, mn: int, mx: int, n_rows: int, bins_max: int = 2000
) -> int:
    """
...
def plot_data_distribution(vals, dist_name, output_path, n_bins=None):
    ...
    try:
        ...
        if n_bins is None:
            fd_n_bins = freedman_diaconis_bins(
                plot_vals,
                int(plot_vals.min()),
                int(plot_vals.max()),
                len(plot_vals),
        ...
        print(
            f"Saved histogram bucket details to {hist_csv_path} (Using {fd_n_bins} FD Bins)"
        )
    except Exception as e:
    ...
    # Finalize Plot
    plt.title(
        f"Distribution: {dist_name} (FD Bins in CSV: {fd_n_bins} | Visual Bins: {plot_bins})"
    )
    plt.xlabel("Value")
    plt.ylabel("Frequency" + (" (Log Scale)" if use_log else ""))
    plt.grid(axis="y", alpha=0.3)
...
def plot_micro_distribution(vals: np.ndarray, dist_name: str, output_path: Path):
    ...
    plt.hist(
        micro_vals, bins=100, color="coral", edgecolor="black", alpha=0.7, log=use_log
    )
    ...
    def ensure_dataset_exists(self, rows: int, dist: str) -> Path:
        ...
        if not (ds_dir / "data.csv").exists() or not (ds_dir / "meta.pkl").exists():
            raise FileNotFoundError(
                f"Dataset {dist} ({rows} rows) not found at {ds_dir}. Please run datasets.py first."
            )
        return ds_dir
    ...
    def prepare_dataset(
        ...
        if not data_path.exists() or force_regeneration:
            ...
            if dist.lower() == "imdb":
                ...
                vals = gen_values(
                    rng, dist, rows, 0, DOMAIN_MAX, use_micro_dist=use_micro_dist
                )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="+",
        help="List of row counts to generate (e.g. 10000 1000000).",
    )
    parser.add_argument(
        "--dist",
        type=str,
        help="Distribution to generate (uniform, normal, zipf, etc).",
    )
    parser.add_argument(
        "--all",
        action="store_true",
        help="Generate all default datasets (1M, 10M, 60M).",
    )
    parser.add_argument(
        "--simple",
        action="store_true",
        help="Use traditional simple approach without micro-distributions.",
    )
