...
def calculate_dataset_stats(rows: int = 10_000_000, output_dir: str = "datasets"):
    ...
    for dist in distributions:
        ...
        _, sample = build_frequency_and_sample(ds_path, mn, mx, N, 100_000, 42)
        ...
        stats_list.append({
            ...
            "Kurtosis": df['v'].kurt(),
            "Bin Count (k)": k,
            "Bin Width (h)": (mx - mn) / k if k > 0 else 0,
            "Min": mn, "Max": mx, "Count (N)": N
        })
        
    stats_df = pd.DataFrame(stats_list)
    out_file = Path(output_dir) / "dataset_statistics.xlsx"
...
def plot_distributions():
    distributions = ["uniform", "normal", "zipf", "sparse_cluster", "anti_zipf"]
    # Implementation depends on data paths, usually called from main.py
    pass
